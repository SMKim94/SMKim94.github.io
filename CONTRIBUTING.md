...
### Microsoft 저작권 자산을 쓰지 않는다

Windows 아이콘을 추출해 넣지 않는다. Fluent 풍으로 **직접 그린다.** 공개
배포되는 사이트라 재배포 문제가 생긴다.
...
- 컬러 앱 아이콘: `viewBox="0 0 48 48"`
- 단색 UI 아이콘: `Mono` 헬퍼를 쓴다 (`viewBox 0 0 24 24`, `currentColor`)

`currentColor`를 쓰면 테마가 바뀔 때 색이 자동으로 따라간다. 이미지로
바꾸면 이게 깨진다.
